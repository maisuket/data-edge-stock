...
import { IngredientService, UNIT_SHORT, type Ingredient } from "@/lib/services/ingredients";
...
export function IngredientDetailDialog({ open, onOpenChange, ingredient }: Props) {
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-lg border border-border bg-muted/30 p-3 text-center">
                ...
                <p className="text-lg font-bold text-foreground tabular-nums mt-1">
                  {data.currentStock.toLocaleString("pt-BR", { maximumFractionDigits: 3 })}
                      ...
                      {data.lots.map((lot) => (
                        <TableRow key={lot.id} className="border-border">
                          ...
                          <TableCell className="text-xs text-muted-foreground">
                            {format(new Date(lot.purchasedAt), "dd/MM/yy", { locale: ptBR })}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
